...
| **`/chats`** | **Mensajeria:** Permite el contacto entre usuarios a la hora de generar el interes por una imagen, para coordinar una venta en privado. |
| **`/colecciones`** | **Colecciones:** Permite gestionar colecciones para posteriormente guardarle a las seleccionadas las publicaciones que el usuario desee. |
| **`/notificaciones`** | **Notificaciones:** Permite la visualizacion de las notificaciones asociadas a diferentes eventos de la aplicacion y actualizar el estado de lectura de cada una. |
| **`/denuncias`** | **Denuncias:** Maneja la creacion de denuncias para comentarios y fotografias, verificando en el segundo caso la regla de 3 denuncias asociadas, a la par permite acceder con permisos de validador a la moderacion de la aplicacion. |
...
Para probar el funcionamiento del sistema se ofrecen los siguientes usuarios
...
* **👤 Usuario:** `albert`
* **🔑 Contraseña:** `12345`
* **🛡️ Rol:** Usuario con permisos de administrador
...
```bash
git clone https://github.com/AleS485/Trabajo-Integrador-WEB2-Fotaza.git
``` 
...
Durante el desarrollo del proyecto los dos problemas que me encontre fueron:
### **1. Subida y procesamiento de imagenes (Avatar y Publicaciones)**
La solucion que encontre y mas me facilito todo el proceso fue realizar esta funcionalidad mediante peticiones AJAX usando fetch() dentro de las vistas PUG las cuales utilizan FileReader para transformar los archivos seleccionados para el envio a base64 para luego enviarlos por JSON al servidor.

### **2. Aplicacion de Marca de Agua en las Fotografias**
En este caso al principio se decidio utilizar la libreria jimp para aplicarle la marca de agua a las fotos, pero a la hora de implementar esta funcion utilizando la documentacion oficial, no funciono de manera adecuada, entregando demasiados errores en el codigo.
La solucion por la que se opto fue cambiar esta libreria por sharp, la cual funciono sin complicaciones y brindando una documentacion oficial mucho mas detallada.
